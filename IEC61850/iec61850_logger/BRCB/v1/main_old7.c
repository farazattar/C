...
reportCallback(void* parameter, ClientReport report)
{
    ...
    for (int i = 0; i < count; i++) {
        MmsValue* entry = MmsValue_getElement(values, i);

		if (MmsValue_getType(entry) == MMS_STRUCTURE) {

			int mcount = MmsValue_getArraySize(entry);

			printf("  [%d] STRUCT (%d members)\n", i, mcount);

			for (int m = 0; m < mcount; m++) {
				MmsValue* member = MmsValue_getElement(entry, m);
				char buf[512];
				mmsValueToString(member, buf, sizeof(buf));
				printf("       - member[%d] = %s\n", m, buf);
			}
		}
		else {
			char buf[512];
			mmsValueToString(entry, buf, sizeof(buf));
			printf("  [%d] %s\n", i, buf);
		}
    }
